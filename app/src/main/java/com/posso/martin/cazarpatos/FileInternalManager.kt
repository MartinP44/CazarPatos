...
class FileInternalManager(val actividad: Activity) : FileHandler {
    override fun SaveInformation(datosAGrabar:Pair<String,String>) {
        ...
        actividad.openFileOutput("fichero-cazarpatos.txt", Context.MODE_PRIVATE).bufferedWriter().use { fos ->
            fos.write(texto)
        }
    }
    override fun ReadInformation():Pair<String,String> {
        try {
            actividad.openFileInput("fichero-cazarpatos.txt").bufferedReader().use {
